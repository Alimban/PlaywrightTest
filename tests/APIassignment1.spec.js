...
async function loginAndGoToEvents(page) {
    ...
    await page.locator('#email').fill("alimbansneakers@gmail.com");

    await page.locator('#password').fill("Cuppycake@1");
...
test("6 event , banner visible", async ({ page }) => {
    ...
    const banner = page.getByText(/sandbox holds up/);

    await expect(banner).toBeVisible();
    await expect(banner).toContainText("6 custom events")

})

// ── Test 2: 4 events → banner is NOT visible

test("4 event , No banner visible", async ({ page }) => {
    ...
    const eventCard = await page.getByTestId('event-card');
    await expect(eventCard.first()).toBeVisible();
    expect(await eventCard.count()).toBe(4);
